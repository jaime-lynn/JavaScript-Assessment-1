...
function iUppercase(string) {
    var sentenceArray = string.split(' ');
    var uppercaseSentence = [];
    for(var i = 0; i < sentenceArray.length; i++){
        if(sentenceArray[i] == 'i') {
            sentenceArray[i] = sentenceArray[i].toUpperCase();
        }
        uppercaseSentence.push(sentenceArray[i]);
    }
    return uppercaseSentence.join(' ');
}
...
function uppercaseScream(string) {
    var sentenceArray = string.split(' ');
    var uppercaseSentence = [];
    for(var i = 0; i < sentenceArray.length; i++){
        if(sentenceArray[i] == 'scream' || sentenceArray[i] == 'scream,'){
            sentenceArray[i] = sentenceArray[i].toUpperCase();
        }
        uppercaseSentence.push(sentenceArray[i]);
    }
